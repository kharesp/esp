...
def extract_features(k,intermediate_node,start,end,log_dir,out_dir):
  ...
  with open('%s/parameterization/k%d'%(log_dir,k),'r') as params,\
    ...
    for idx,param_str in enumerate(params):
      if (idx+1)<start:
        continue
      elif (idx+1)>end:
        break
        ...
        path_latency={}
        with open('%s/k%d/%d/summary/summary_g1.csv'%(log_dir,k,idx+1),'r') as latf:
          next(latf)
          for line in latf:
        ...
        #if os.path.exists('log/model_learning/k%d/%d/summary/summary_util.csv'%(k,idx+1)):
        #  with open('log/model_learning/k%d/%d/summary/summary_util.csv'%(k,idx+1),'r') as uf:
        #    next(uf)
        #    for line in uf:
